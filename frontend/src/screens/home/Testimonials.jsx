...
const Testimonials = () => {
  ...
  return (
    <section className="py-5 px-4 md:px-8 lg:px-10 xl:px-16 relative font-dm-sans">
      ...
      <div
        ref={boxRef}
        className="mx-auto max-w-screen-2xl px-4 py-5 sm:px-6 xl:px-8 xl:py-16"
      >
        <Slider {...settings}>
          {data.map((testimonial, index) => (
            <div className="p-4 h-full flex" key={index}>
              <blockquote className="flex flex-col justify-between bg-white p-6 shadow-sm rounded-lg w-full max-width-[360px] sm:max-w-none min-h-[300px] sm:min-h-[350px] xl:min-h-[320px]">
                <article>
                ...
                <p className="mt-4 text-sm flex-grow">{testimonial.des}</p>
              </blockquote>
            </div>
          ))}
        </Slider>
