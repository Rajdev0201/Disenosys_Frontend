...
import faq from "@/components/assests/faq.jpg";
...
const FaqSection = () => {
  ...
  return (
    <div className="lg:mt-25">
      {/* TOP SECTION */}
      <div className="w-full flex flex-col md:flex-row font-dm-sans justify-center items-center text-center mt-20 px-4 gap-10">
        <Image
          src={faq}
          alt="faq"
          className="object-cover w-3/4 lg:-ml-60 sm:w-52 md:w-64 mb-4 flex"
        />

        <div>
          <p className="text-sm font-bold text-[#141554] tracking-tighter mb-2">
            FAQs
          </p>
          <h1 className="text-3xl sm:text-4xl tracking-tighter font-bold text-[#141554]">
            Have Questions?{" "}
            <span className="text-[#32C7FF]">We’ve Got Answers</span>
          </h1>
          <p className="text-[#8C8C8C] mt-3 text-base sm:text-lg">
        ...
        <div className="mx-4 sm:mx-10 md:mx-20 lg:mx-32 max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 lg:gap-12 auto-rows-fr">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className="bg-white rounded-lg shadow-lg p-6 hover:shadow-md transition-shadow h-full flex flex-col cursor-pointer"
                onClick={() => toggleFAQ(i)}
              >
                <div
      ...
      {/* Bottom*/}
      <div>
        <div className="bg-[#141554] mb-6 flex flex-col md:flex-row items-center justify-between p-8 rounded-lg mx-4 sm:mx-10 lg:mx-32 mt-10 gap-4">
          ...
          <Link
            href="/contact"
            className="flex bg-[linear-gradient(to_right,#0077B3,#45D2FF)] text-[#FFFFFF] px-6 py-2 rounded-full"
          >
            Get in touch
          </Link>
        </div>
