...
const Curriculum = ({course}) => {
  ...
  return (
    <div className="py-4 font-dm-sans">
      <div>
            ...
            {course?.Curriculum?.map((item, idx) => (
              <div key={idx} ref={(el) => (courseRefs.current[idx] = el)}>
                ...
                ""
                  ...
                  className="w-full text-left bg-gray-100 my-2 p-2 hover:cursor-pointer mt-4 rounded-md hover:bg-gray-200 focus:outline-none flex items-center justify-between"
                >
                  <span className="font-bold text-lg text-[#101359] ">Module-{idx + 1}: {item.title}</span>
                  <span className="text-[#101359] ">
                    ...
                    <ul className="list-disc space-y-2 px-1">
                        {item.subTopics && (
                          (Array.isArray(item.subTopics)
                            ...
                            <li key={subIdx} className="text-md font-semibold ">
